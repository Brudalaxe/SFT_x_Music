...
class TextClassificationDataset(Dataset):
    ...
    def build_label_index(self):
        ...
        self.label2id['not_entailment'] = 1
        self.id2label[0] = "entailment"
        self.id2label[1] = "not_entailment"
    ...
    def read_file(self, data_path, max_length, need_label=True):
        ...
        for line in tqdm(data, desc="processing data", leave=False):
            _, question, sentence, label = line.split('\t')
            if label not in self.label2id:
                error_line_nums += 1
                continue

            encode_text = self.tokenizer(question, text_pair=sentence)
