...
for e in range(args.epoch):
    ...
    for batch in tqdm(train_loader, desc="eval train data", leave=False):
        ...
        with torch.no_grad():
            ...
            pred = torch.argmax(y_hat, dim=-1).detach().cpu().tolist()
    ...
    logger.info(f"epoch: {e} train set loss: {dev_loss / all_num} mcc: {matthews_corrcoef(y_true, y_pred)}")
    ...
    logger.info(f"epoch: {e} dev set loss: {dev_loss / all_num} mcc: {matthews_corrcoef(y_true, y_pred)}")
