...
class SvdLayer(nn.Module):
    def __init__(self, layer, rank):
        ...
        # self.LayerNorm.weight = nn.Parameter(layer.LayerNorm.weight * 1.414)
        # self.LayerNorm.bias = nn.Parameter(layer.LayerNorm.bias)
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load("sst2.pt", map_location="cpu"))
    # model.eval()
    # print(model.bert.encoder.layer[5].output.LayerNorm)
    # print(model(**encode_text))
    #
    # layer = model.bert.encoder.layer[5].output
    # model.bert.encoder.layer[5].output = SvdLayer(layer, rank=1)
    print(model.bert.encoder.layer[5])
    # print(model(**encode_text))
    # weight = model.bert.encoder.layer[0].output.LayerNorm.weight.detach().numpy().flatten()
    # plt.hist(weight, bins=200, range=(0.5, 1))
    # plt.show()
